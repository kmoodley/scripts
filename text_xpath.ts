describe('test xpath select', () => {
    ...
    it('test additionalConditions', async () => {
        ...
        {
            const nodeValue = validateOptions(data.responseXML,data.successPath)
            expect(nodeValue).to.eql(data.value);
        }
    });
    ...
    function validateOptions(responseStr : string , condtionStr : string)
    {
        ...
        return xpath.select1(condtionStr, response)?.nodeValue;
    }
});
